...
namespace socketify
{
  ...
  bool Router::dispatch(Request &req, Response &res) const
  {
    ...
    {
      // Try path match first (independent of method)
      Request temp = req;
      ...
      {
        // record allowed method for same path
    ...
    if (!matched)
    {
      if (path_matched_any)
      {
        // Build Allow header (unique, sorted-ish; include HEAD if GET present)
        std::sort(allowed.begin(), allowed.end(), [](Method a, Method b)
      ...
      return false; // truly no path matched → 404 handled by caller
    }

    // Build middleware chain: global -> group -> route -> handler
